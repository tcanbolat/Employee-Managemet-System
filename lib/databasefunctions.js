...
const updateRole = () => {
  ...
  connection.query(`SELECT title FROM role; SELECT concat(employee.first_name, " ", employee.last_name) AS Name, role.title
  FROM employee JOIN role ON (employee.role_id = role.id);`, function (err, res) {
...
inquirer.prompt([
  ...
  const employeeFirstName = answers.employeeChoice.split(' ').slice(0, -1).join(' ');
  const employeeLastName = answers.employeeChoice.split(' ').slice(-1).join(' ');

  connection.query(`UPDATE employee 
        SET role_id = (SELECT id FROM (SELECT * FROM role) AS A WHERE title = "${answers.newRole}") 
        WHERE id = (SELECT id from (SELECT * FROM employee) AS A 
        WHERE first_name = "${employeeFirstName}" 
        AND last_name = "${employeeLastName}");`, function(err, res) {
          if (err) throw err;
          console.log("\n");
          console.table(res);
          console.log("\n");
